""" test with random generated points for ransac filtering and 
    LSM fitting of geometric shapes:
...
if args.single_test in ('all', 'line2d'):
    # test for line
    ...
    en = np.c_[east, north]
    en += np.random.rand(*en.shape) * noise / (2**0.5)   # add noise
    lr = LinearReg(en)
    r = Ransac(lr)
    en_fit, iterations = r.ransac_filter(tolerance=ransac_limit, iterations=iterations)
    en_line = lr.get_pnts_by_index(en_fit)
    print("-" * 80)
    print("2D line")
    print(f"{en_line.shape[0]}/{en.shape[0]} points fit")
    print(f"Original   params: {param_0[0]:.5f} {param_0[1]:.5f} {param_0[2]:.3f}")
    print(f"Params after RANS: {lr.params[0]:.3f} {lr.params[1]:.3f} {lr.params[2]:.3f}")
...
if args.single_test in ('all', 'plane'):
    # test for plane
    ...
    enz_plane = lr.get_pnts_by_index(enz_fit)
    print("-" * 80)
    print("Plane")
    ...
    print(f"Params after RANS: {lr.params[0]:.3f} {lr.params[1]:.3f} {lr.params[2]:.3f} {lr.params[3]:.3f}")
    # calculate final params
...
if args.single_test in ('all', 'circle'):
    # test for circle
    ...
    if args.plot:
        ...
        circle = patches.Circle(params[:2], params[2], color='blue', linewidth=2, fill=False)
        ax.add_patch(circle)
...
if args.single_test in ('all', 'sphere'):
    # test for sphere
    ...
    r = Ransac(sr)
    enz_fit, iterations = r.ransac_filter(tolerance=ransac_limit, iterations=iterations)
    enz_sphere = sr.get_pnts_by_index(enz_fit)
    print("-" * 80)
    print("Sphere")
    ...
    print(f"Params after RANS: {sr.params[0]:.3f} {sr.params[1]:.3f} {sr.params[2]:.3f} {sr.params[3]:.3f}")
    # calculate final params
    ...
    if args.plot:
        ...
        ax.set_title(f"{enz_sphere.shape[0]}/{enz.shape[0]} points fit")
        ax.view_init(32, 60)
        ax.set_aspect('equal')
        plt.show()

if args.single_test in ('all', 'ellipse'):
    # test for ellipse
    ...
    r = Ransac(er)
    en_fit, iterations = r.ransac_filter(tolerance=ransac_limit, iterations=iterations)
    en_ellipse = er.get_pnts_by_index(en_fit)
    print("-" * 80)
    print("Ellipse")
    ...
    print(f"Params after RANS: {er.params[0]:.3f} {er.params[1]:.3f} {er.params[2]:.3f} {er.params[3]:.3f} {er.params[4]:.3f}")
    # calculate final params
...
if args.single_test in ('all', 'line3d'):
    # 3D line
    ...
    r = Ransac(lr)
    enz_fit, iterations = r.ransac_filter(tolerance=ransac_limit, iterations=iterations)
    enz_line = lr.get_pnts_by_index(enz_fit)
    print("-" * 80)
    print("3D line")
    ...
    print(f"Params after RANS: {lr.params[0]:.3f} {lr.params[1]:.3f} {lr.params[2]:.3f} {lr.params[3]:.3f} {lr.params[4]:.3f} {lr.params[5]:.3f}")
    # calculate final params
