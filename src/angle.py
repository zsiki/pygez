...
def _pdeg2rad(angle: float) -> float:
...
def _rad2pdeg(value: float) -> float:
...
def _rad2mil(value: float) -> float:
...
class Angle():
    ...
    def __format__(self, spec: str) -> str:
        """ returns formatted angle string
            :param: spec format spcifier DMD/DEG/GRAD/...
        """
        if spec is None or spec == 'RAD':
            return f"{self.value:.7f}"
        elif spec in self.ex:
            return str(self.ex[spec](self.value))
